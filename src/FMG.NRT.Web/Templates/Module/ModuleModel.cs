...
namespace FMG.NRT.Web.Templates
{
    public class ModuleModel
    {
        ...
        public ModuleModel(String model, String controller, String area)
        {
            ...
            ControllerTestsNamespace = "FMG.NRT.Tests.Unit.Controllers" + (!String.IsNullOrWhiteSpace(area) ? $".{area}" : "");
            ControllerNamespace = "FMG.NRT.Controllers" + (!String.IsNullOrWhiteSpace(area) ? $".{area}" : "");
            ControllerTests = $"{controller}ControllerTests";
            Controller = $"{controller}Controller";

            Area = area;

            Type type = typeof(BaseModel).Assembly.GetType("FMG.NRT.Objects." + model) ?? typeof(BaseModel);
